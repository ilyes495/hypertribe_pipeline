...
const char* HomerConfig::homeDirectory = "/data/kharas/baalii/HyperTribe_SYNCRIP_MOLM13/download_scripts/../software/hypertribe/homer/";
...
char* cleanDirectoryName(char* path) {
	int len = strlen(path);
	if (path== NULL || len < 1) return NULL;
...
PeakLibrary* PeakFinder::findmCPeaks() {
	...
	int randFlag = 0;
	PeakLibrary* peaks = tags->findmCPeaks(peakSize, strand, mCflag, methylCthreshold, minNumC, input,randFlag);
	numPeaks = peaks->numPeaks;

	randFlag = 1;
	//fprintf(stderr, "\tRandomizing data and trying again to estimate the FDR...\n");
	//PeakLibrary* peaksRand = tags->findmCPeaks(peakSize, strand, mCflag, methylCthreshold, minNumC, input,randFlag);
	//int numPeaksRand = peaksRand->numPeaks;
	//fprintf(stderr, "\tFDR estimate: %.2lf%% (%d/%d)\n", ((double)numPeaksRand)/((double)numPeaks)*100.0,
	//				numPeaksRand,numPeaks);
...
PeakLibrary* PeakFinder::findPeaks() {
	...
	if (inputPeaksFile == NULL) {
		...
		if (regionFlag && style != PEAK_STYLE_SUPERENHANCERS) {
			...
			//if (localFold > 0) {
			//Old placement - now moved infront of the stiching
			if (0 && localFold > 0) {
				int before = filteredPeaks->numPeaks;
				tags->setMaxTBP(0);
				addHeader((char*)"Fold Change vs Local");
		...
		fprintf(stderr, "\tUsing provided input peaks to find super enhancers with\n");
		filteredPeaks = new PeakLibrary(inputPeaksFile,PEAK_READ_MODE_NORMAL);
	}
	...
	if (style == PEAK_STYLE_SUPERENHANCERS && input != NULL) {
		...
		Doubletable* inputTags = filteredPeaks->countPeakTagsLowMemory(input,strand,COUNT_MODE_TOTAL,excludePeaks);
...
PeakLibrary* PeakFinder::filterPeaks(PeakLibrary* putativePeaks) {
	...
	for (int i=fdrSize-2;i>=0;i--) {
		...
		if (cfdr > fdr/10000.0) {// && i <= 50) {
			...
			fprintf(stderr, "\t\t%d\t%.3lf\t%.3lf\t%.2le\t%.2le\n", i,peakHeights[i],fdrTable[i],cfdr,poissonTable[i]);
			//fprintf(stderr, "\t\t%d\t%.3lf\t%.3lf\t%.2le\n", i,peakHeights[i],fdrTable[i],cfdr);
		}
	}
...
void PeakLibrary::processBedGraphFile(char* bedGraphFile, int relativeStart, int relativeEnd,
	...
	if (fp == NULL) {
		fprintf(stderr, "!!! Could not open bedGraph file: %s!!!\n", bedGraphFile);
		exit(0);
	}
	while (fgets(buf, BUFFER, fp) != NULL) {
	...
	for (int i=0;i<linkedFirstTags->total;i++) {  
		...
		Peak* peak = (Peak*) peaks->search(keys[i]);
		if (peak == NULL) continue;
		int plength = peak->end - peak->start + 1;
		...
		if (outputMode == OUTPUT_MODE_COUNT) {
			double actualSize = (double)plength;
			//fprintf(stderr, "actualSize=%lf\n", actualSize);
			//actualSize = totalBp;
			if (actualSize > 0.0) {
				total /= actualSize;
			}
...
void PeakLibrary::processWiggleFile(char* wiggleFile, int relativeStart, int relativeEnd,
	...
	if (fp == NULL) {
		fprintf(stderr, "!!! Error could not open wiggle file: %s!!!\n", wiggleFile);
		exit(0);
	}
	while (fgets(buf, BUFFER, fp) != NULL) {
		split(buf, line, numCols, WHITE_SPACE);
	...
	for (int i=0;i<linkedFirstTags->total;i++) {  
		...
		if (outputMode == OUTPUT_MODE_COUNT) {
			if (plength > 0) total /= (double)plength;
...
PeakLibrary* PeakLibrary::subtractPeaks(PeakLibrary* exclude) {
...
void PeakLibrary::printBED2D(FILE* fp) {
	char** keys = peaks->keys();
	for (int i=0;i<numPeaks;i++) {
		//fprintf(stderr, "\t%d\t%s\n", i, keys[i]);
	}
...
void PeakLibrary::printCurrentPeakOrder(FILE* fp) {
	for (int i=0;i<numPeaks;i++) {
		peakOrder[i]->print(fp);
	}	
}
...
Doubletable* PeakLibrary::countPeakTagsLowMemory(TagLibrary* tags, char direction,int mode,PeakLibrary* excludePeaks) {
...
void PeakLibrary::setDefaultPeakOrder(int mode) {
	if (peakOrder != NULL) {
		delete []peakOrder;
	}
	if (numPeaks < 1) return;
	...
	if (mode == PEAK_SORT_BY_CHR_START_COORDINATE) { //0
		qsort(peakOrder,numPeaks,sizeof(Peak*),&cmpPeaks);
	} else if (mode == PEAK_SORT_BY_CHR_END_POS_COORDINATE) {
		qsort(peakOrder,numPeaks,sizeof(Peak*),&cmpPeaksEndPos);
	}
	for (int i=0;i<numPeaks;i++) {
		peakOrder[i]->index = i;
	}
...
void PeakLibrary::annotatePeakLocations(PeakLibrary* annotations, FILE* statsFile, FILE* annFile) {
	...
	if (statsFile != NULL) {
		fprintf(statsFile,"Annotation\tNumber of peaks\tTotal size (bp)\tLog2 Ratio (obs/exp)\tLogP enrichment (+values depleted)\n");
	}
	fprintf(stderr,"\t\tAnnotation\tNumber of peaks\tTotal size (bp)\tLog2 Ratio (obs/exp)\tLogP enrichment (+values depleted)\n");
	...
	for (int i=0;i<sizeTotals->total;i++) {
		...
		double obsFraction = 0.0;
		double ratio = 0.0;
		if (nn < 1) {
			nn = 1-expectedFraction;
			obsFraction = nn/totalregions;
			ratio = obsFraction/(expectedFraction+obsFraction);
		} else {
			obsFraction = nn/totalregions;
			ratio = obsFraction/expectedFraction;
		}
		if (ratio > 1e-200) {
			ratio = log(ratio)/log(2.0);
		}
		double logp=logbinomialD(totalregions,nn,expectedFraction,INT_MAX);
		if (ratio < 0) logp=-1*ilogbinomialD(totalregions,nn,expectedFraction,INT_MAX);

		if (statsFile != NULL) {
			fprintf(statsFile,"%s\t%.1lf\t%.0lf\t%.3lf\t%.3lf\n",annname,n,s,ratio,logp);
		}
		fprintf(stderr,"\t\t%s\t%.1lf\t%.0lf\t%.3lf\t%.3lf\n",annname,n,s,ratio,logp);
		delete [](keys[i]);
	}
	delete []keys;
	delete stats;
...
void PeakLibrary::getCoBoundPeaks(char** peakFiles, int numPeakFiles, char strand, int maxDistance, 
	...
	char** keys = peaks->keys();
	char* peakOverlapStr = new char[1000000];
	setDefaultPeakOrder();
	...
	for (int i=0;i<numPeakFiles;i++) {
		...
		//fprintf(stderr, "totalOverlap=%lld\n", totalOverlap);
 		for (int j=0;j<numPeaks;j++) {
			peakOverlapStr[0] = '\0';
			char* pname = peakOrder[j]->name;

			PeakMapping* pm = (PeakMapping*) mapping->search(pname);
			if (pm != NULL) {
				//there was an overlap for this peak
				int co = cobound->search(pname);
				if (co == EMPTY_INT) {
					fprintf(stderr, "\tSomething might be wrong....\n");
				}	
				//update co-bound count
				cobound->insert(co+1,pname);

				//add overlapping peak identities to reporter string
				for (int k=0;k<pm->numPeaks;k++) {
					if (k>0) {
						strcat(peakOverlapStr,",");
					}
					strcat(peakOverlapStr, pm->peaks[k]->name);
					//fprintf(stderr,"%s,", pm->peaks[k]->name);
				}
				delete pm;
			}
			peakOrder[j]->addData(peakOverlapStr);
		}
		/*
		char** mkeys = mapping->keys();
		for (int j=0;j<mapping->total;j++) {
	...
	for (int i=0;i<maxCoBound+1;i++) {
		...
		fprintf(fps[i],"#Name (cmd = %s)\tchr\tstart\tend\tstrand\tstat\tstat2",cmdline);
		for (int j=0;j<numPeakFiles;j++) {
			fprintf(fps[i],"\t%s",peakFiles[j]);
		}
		fprintf(fps[i],"\n");
	}
	delete []filename;
...
int cmpPeaksEndPos(const void* a, const void* b) {
	char* ac = (*(Peak**)a)->chr;
	char* bc = (*(Peak**)b)->chr;
	int cc = chrcmp(&ac,&bc);
	if (cc != 0) return cc;
	...
	for (int i=0;i<numPeaks;i++) {
		...
		for (int j=1;j<numPeaks;j++) {
			...
			if (maxDistance < 0) { //given overlap
				...
				if (fabs((double)(peaks[j]->refPos-peaks[i]->refPos)) < maxDistance) {
					if (peaks[j]->end > peaks[i]->end) {
						peaks[i]->end = peaks[j]->end;
					}
					mask[j]=1;
...
void Peak::printBED2D(FILE* fp) {
	fprintf(fp, "%s\t%d\t%d\t%s\t%d\t%d",chr,start-1,end-1,chr,start-1,end-1);
	fprintf(fp, "\t255,255,0\t%.3f\t%.3f", v, focusRatio);
	fprintf(fp, "\n");
}
...
void TagLibrary::parseAlignmentFiles(char** files, int numFiles, int format, int mode,
							char** tagDirs, int numDirs,char** tagFiles,int numTagFiles,int rmsoftFlag) {
	...
	for (int i=0;i<numFiles;i++) {
		...
		if (pairedEndFlag) {
			...
			readPEAlignment(files[i],format,mode,rmsoftFlag);
		} else {
			int zippedFlag = 0;
			...
			readAlignment(workingFilename,currentFormat,mode,TAGLIBRARY_SINGLE_FLAG,rmsoftFlag);
...
void TagLibrary::readPEAlignment(char* file, int format, int mode,int rmsoftFlag) {
	...
	//fprintf(stderr, "\tReading alignment file %s\n", files[i]);
	readAlignment(workingFilename,currentFormat,mode,TAGLIBRARY_PE_READ1_FLAG,rmsoftFlag);
	rezipFileIfNeeded(workingFilename,zippedFlag);
	zippedFlag = 0;
	currentFormat = format;
	workingFilename = unzipFileIfNeeded(files[1],zippedFlag,currentFormat);
	//fprintf(stderr, "\tReading alignment file %s\n", files[i]);
	readAlignment(workingFilename,currentFormat,mode,TAGLIBRARY_PE_READ2_FLAG,rmsoftFlag);
	rezipFileIfNeeded(workingFilename,zippedFlag);
...
void TagLibrary::trimIlluminaReadName(char* name) {
	...
	if (slen > 2) {
		// for cases where the read number is kept in the name like ABCD/0 and ABCD/1
		if (name[slen-2] == '/') {
			name[slen-2] = '\0';
			slen -= 2;
		}
	...
	if (1) {
		// for cases where the read number is kept in the name like ABCD/0 and ABCD/1
		// NB501406:12:HKW7JBGXX:1:11101:2918:1142_1:N:0:CACTCA
		int numColon = 0;
...
void TagLibrary::randomizeReadLevelsPerChr() {
...
void TagLibrary::readAlignment(char* file, int format, int mode, int PEflag,int rmsoftFlag) {
	...
	while (fgets(buf, BUFFER, fp) != NULL) {
		...
		if (format == FORMAT_HICSUMMARY) {
			...
			int startPos = getRightCoordFromCIGAR(line[5],dir,cigarCodes,cigarLens,numCodes,initLen,rmsoftFlag);
			if (startPos == CIGAR_OMIT) continue;
...
int TagLibrary::getRightCoordFromCIGAR(char* str, int dir, char* cigarCodes, int* cigarLens, int &numCodes, int &initLen,int rmsoftFlag) {
	numCodes = 0;
	int i = 0;
	char* start = str;
	int lastStartI = 0;
	...
	while (str[i] != '\0') {
		if (str[i] < 48 || str[i] > 57) {
			...
			if ((rmsoftFlag == PARSE_SAM_RMSOFT_DELETESOFT) && ((cigarCodes[numCodes] == 'S') || (cigarCodes[numCodes] == 'N'))) {
				return CIGAR_OMIT;
			}
			if (cigarCodes[numCodes] == 'M' || cigarCodes[numCodes] == '=') {
				...
				firstActive = 0;
				lastActive = 0;
			} else if (cigarCodes[numCodes] == 'I') {
				totalLen -= cigarLens[numCodes];
			} else if (cigarCodes[numCodes] == 'S' && rmsoftFlag == PARSE_SAM_RMSOFT_EXTEND) {
				totalLen -= cigarLens[numCodes];
			}
			start = &(str[i+1]);
			lastStartI = i+1;
	...
	if (mCflag==0) {
		...
		if (posVsNegEnrichment > AUTOCORRELATION_RNA_THRESHOLD) {
			...
			if (negEnrichment <= 0.01 || fabs(log(posEnrichment/negEnrichment))>0.50) {
				...
				fprintf(stderr, "\t\tGuessing sample is ChIP-Seq or unstranded RNA-Seq - autocorrelation looks good.\n");
			}
		} else {
...
PeakLibrary* TagLibrary::findmCPeaks(int peakSize, char strand,int mCflag, double mCthresh, 
										int minNumC, TagLibrary* input, int randFlag) {
	...
	for (int i=0;i<chrs->total;i++) {
		...
		if (input != NULL) {
			...
			if (ctinput == NULL) continue;
			ctinput->loadTags();
		}
		ct->findmCPeaks(peaks,peakSize,strand,mCflag,mCthresh, minNumC,ctinput,randFlag);
	}
	fprintf(stderr, "\n\tTotal Regions = %d\n\n", peaks->numPeaks);
	return peaks;
}
...
void TagLibrary::printBedGraph(FILE* fp, double normTotalTags, char strand, int resolution, int negFlag,
			...
			TagLibrary* input, double pseudoCounts, int logFlag, int subtractFlag, double normLength, 
			char** skipChr,int numSkipChr, int noHeaderFlag) {
	char* cc = color;
	int c1 = 0;
	int c2 = 0;
	int c3 = 0;
	...
	char** keys = chrs->keys();
	qsort(keys,chrs->total,sizeof(char*),&strcmpForQsort);
	//qsort(keys,chrs->total,sizeof(char*),&chrcmp);


	if (strand == STRAND_SEPARATE || strand == STRAND_POSITIVE || strand==STRAND_BOTH) {
		if (method == UCSC_BEDGRAPH && !circosFlag && noHeaderFlag==0) {
		...
		} else if (method == UCSC_BIGWIG && !circosFlag && noHeaderFlag==0) {
			if (fp != stdout) {
		...
		for (int i=0;i<chrs->total;i++) {
			...
			int skip=0;
			for (int j=0;j<numSkipChr;j++) {
				if (strcmp(keys[i],skipChr[j])==0) {
					skip=1;
					break;
				}
			}
			...
			ct->printBedGraph(fp,normFactor,curStrand,resolution,negFlag,fragmentLengthEstimate,
										...
										inputct, pseudoCounts, logFlag,subtractFlag,inputNormFactor,inputFragLength,
										normLength);
	...
	if (strand == STRAND_SEPARATE || strand == STRAND_NEGATIVE) {
		...
		for (int i=0;i<chrs->total;i++) {
			...
			ct->printBedGraph(fp,normFactor,curStrand,resolution,negFlag,fragmentLengthEstimate,
										reductionRatio,style,condenseFlag,lastTagFlag,umc,circosPeak,
										inputct, pseudoCounts, logFlag,subtractFlag,inputNormFactor,
										inputFragLength,normLength);
			if (umc != NULL) delete umc;
...
void TagLibrary::annotateTagLocations(PeakLibrary* annotations, FILE* statsFile, FILE* annFile) {
	...
	if (statsFile != NULL) {
		fprintf(statsFile,"Annotation\tNumber of reads\tTotal size (bp)\tLog2 Ratio (obs/exp)\tLogP enrichment (+values depleted)\n");
	}
	fprintf(stderr,"\t\tAnnotation\tNumber of reads\tTotal size (bp)\tLog2 Ratio (obs/exp)\tLogP enrichment (+values depleted)\n");
...
//xxxxxxxxxxxxxxxxxxx
void TagLibrary::normalizeTagCountsLength(char* lenCtrlFile,double minNormRatio, double maxNormRatio, double maxPercentError) {

	//hash table will be filled with double array 3 units long (observed,expected,adjustRatio)
	double** readLenTable = new double*[MAX_READ_LENGTH];
	...
	FILE* fp = fopen(lenCtrlFile, "r");
	if (fp == NULL) {
		fprintf(stderr, "!!!Could not open read length distribution file: %s!!!\n",lenCtrlFile);
		return;
	}
	int lineNum = 0;
	while (fgets(buffer, BUFFER, fp) != NULL) {
		...
		sscanf(line[0],"%d",&curLen);
		sscanf(line[1],"%lf",&curFrac);
		if (curLen < 0 || curLen >= MAX_READ_LENGTH) {
			fprintf(stderr, "!!! Warning - read length out of range: %d\n", curLen);
			continue;
		}
		if (curLen < minLength) minLength = curLen;
		if (curLen > maxLength) maxLength = curLen;
		readLenTable[curLen][1] = curFrac;
	}
	fclose(fp);

	char* filename = getDirFileName((char*)"tagLengthDistribution.txt");
	fp = fopen(filename, "r");
	if (fp == NULL) {
		fprintf(stderr, "!!!Could not open read length distribution file: %s!!!\n",filename);
		return;
	...
	while (fgets(buffer, BUFFER, fp) != NULL) {
		...
		readLenTable[curLen][0] = curFrac;
	}
	fclose(fp);
	delete []filename;

	//calculate adjustment ratios for each read length
	filename = getDirFileName((char*)"tagLengthNormalization.txt");
	fp = fopen(filename,"w");
	int maxIterations = 50;
	//double lastError = FLT_MAX;
	double pseudoFrac = 0.00001;
	for (int i=0;i<maxIterations;i++) {

		//double perror = 0;
		fprintf(fp, "\tLen\tObs\tExp\tRatio\n");
		for (int i=minLength;i<=maxLength;i++) {
			double ratio = (readLenTable[i][1]+pseudoFrac)/(readLenTable[i][0]+pseudoFrac);
			if (ratio > maxNormRatio) ratio = maxNormRatio;
			if (ratio < minNormRatio) ratio = minNormRatio;
			readLenTable[i][2] = ratio;
			fprintf(fp, "\t%d\t%lf\t%lf\t%lf\n", i,readLenTable[i][0],readLenTable[i][1],readLenTable[i][2]);
		}
		break;
	}
	fclose(fp);
	...
	for (int i=0;i<chrs->total;i++) {
		ChrTags* ct = (ChrTags*)chrs->search(keys[i]);
		ct->normalizeTagCountsLength(readLenTable);
		delete [](keys[i]);
		totalTags += ct->totalTags;
	}
	delete []keys;
...
void TagLibrary::filterReadsWithOligo(char* genomeDirectory, char* filterOligo, 
															int filterOffset, int filterFlag) {
	fprintf(stderr, "\tFiltering reads based on presense of a sequence\n");
	double og = totalTags;
	totalTags = 0.0;
	char** keys = chrs->keys();
	qsort(keys,chrs->total,sizeof(char*),&chrcmp);
	...
	for (int i=0;i<chrs->total;i++) {
		ChrTags* ct = (ChrTags*)chrs->search(keys[i]);
		ct->filterReadsWithOligo(genomeDirectory, filterOligo, filterOffset, filterFlag);
		delete [](keys[i]);
		totalTags += ct->totalTags;
	}
	...
	delete []keys;
	fprintf(stderr, "\tKept %.1lf of %.1lf (%.2lf%%)\n",totalTags,og,totalTags/og*100);
}
...
PeakLibrary* TagLibrary::getCoveragePeaks(char* chr, int start, int end, int resolution,int superRes) {
	...
	for (int i=0;i<chrs->total;i++) {
		...
		for (int j=cstart;j<cend;j+=resolution) {
			char* pname = new char[strlen(keys[i])+200];
			sprintf(pname,"%s-%d",keys[i],j); 
			int midpoint = j+resolution/2;
			int s = midpoint - halfSuperRes;
			int e = midpoint + halfSuperRes;
...
double TagLibrary::estimateContamination(TagLibrary* input, int maxDistance, float minThreshold) {
	...
	Doubletable* targetCounts = peaks->countPeakTagsLowMemory(this,POSITIVE_STRAND,COUNT_MODE_TOTAL,NULL);
	Doubletable* inputCounts = peaks->countPeakTagsLowMemory(input,POSITIVE_STRAND,COUNT_MODE_TOTAL,NULL);
...
PEFreqDistribution::PEFreqDistribution(TagLibrary* tags) {
	...
	filename = new char[strlen(tagDir->directory)+50];
	sprintf(filename, "%s/petag.FreqDistribution_1000.txt", tagDir->directory);
	readPEFreqFile(filename);

}
PEFreqDistribution::~PEFreqDistribution() {
	if (filename != NULL) {
		delete []filename;
	}
	if (fileFreq != NULL) { 
...
void PEFreqDistribution::readPEFreqFile(char* file) {
	char* buf = new char[BUFFER];
	char** line = new char*[BUFFER];
	int numCols=0;

	fileFreq = new double[1000000];
	fileFreqInterChr = 0.0;
	for (int i=0;i<1000000;i++) fileFreq[i] = 0.0;
	fileFreqRes = 1000;
	fileFreqLength = 0;
	...
	while (fgets(buf, BUFFER, fp) != NULL) {
		...
		if (line[0][0] == 'M') break; //skip last line
		int distance = -1;
		double frequency = -1.0;
		sscanf(line[0],"%d",&distance);
		sscanf(line[1],"%lf",&frequency);
		totalFreq+=frequency;
		fileFreq[fileFreqLength++] = frequency;
	}
	fclose(fp);
	delete []buf;
	delete []line;

	fileFreqInterChr = 1.0-totalFreq;

	fprintf(stderr, "\tFinished reading average contact frequency, interchr = %lf\n", fileFreqInterChr);

}
void PEFreqDistribution::setRes(int r) {
	freqRes = r;
	if (freqRes < fileFreqRes) {
		fprintf(stderr, "!!!Error - Unfortunately, resolution(%d) needs to equal or larger than the default (%d)\n",
					freqRes, fileFreqRes);
		exit(0);
	}
	if (freq != NULL) delete []freq;
	if (fileFreq == NULL) {
		fprintf(stderr, "!!!Error - the petag.FreqDistribution.txt file doesn't appear to be loaded!!!\n");
		exit(0);
	}

	freqInterChr = fileFreqInterChr;
	freqLength = (fileFreqLength*fileFreqRes)/freqRes+10;
	freq = new double[freqLength];
	for (int i=0;i<freqLength;i++) freq[i]=0.0;

	int fileIndex =0;
	for (int i=0;i<freqLength;i++) {
		freq[i] = 0.0;
		int maxDist = i*freqRes + (freqRes/2);

		while (fileIndex < fileFreqLength) {
			int p = fileIndex*fileFreqRes;
			if (p > maxDist) break;
			freq[i] += fileFreq[fileIndex];
			fileIndex++;
		}
	}
}
void PEFreqDistribution::printFreq(FILE* fp) {
	fprintf(fp, "Distance\tFrequency(Interchromosomal:%e\n", freqInterChr);
	for (int i=0;i<freqLength;i++) {
		int d = freqRes*i;
		double f = freq[i];
		fprintf(fp, "%d\t%e\n",d,f);
...
//
void TagLibrary::getPETagTotals(PeakLibrary* peaks, int balanceEstFlag) {
	...
	if (balanceEstFlag) {
		balancePETagTotals(peaks);
	}
}
void TagLibrary::balancePETagTotals(PeakLibrary* peaks) {

	double dampFactor = 4.0;

	int r = peaks->peakOrder[1]->start - peaks->peakOrder[0]->start;
	int sr = peaks->peakOrder[0]->end - peaks->peakOrder[0]->start;
	fprintf(stderr, "\tInferred res=%d, superRes=%d\n", r,sr);

	PEFreqDistribution* dist = new PEFreqDistribution(this);
	dist->setRes(sr);

	double total = 0;
	double freqTotal = 0;
	double *adjTotals = new double[peaks->numPeaks];
	double *newAdjTotals = new double[peaks->numPeaks];
	for (int i=0;i<peaks->numPeaks;i++) {
		adjTotals[i]=1.0;
		newAdjTotals[i]=1.0;
		total += peaks->peakOrder[i]->v;
		freqTotal += dist->freq[i];
		//peaks->peakOrder[i]->print(stdout);
	}
	//dist->printFreq(stdout);
	
	double avg = total/((double)peaks->numPeaks);
	avg *= freqTotal;
	fprintf(stderr, "\tavg coverage=%lf (n=%d, freqTotal=%lf)\n", avg,peaks->numPeaks,freqTotal);

	double error = 1e10;
	double minError = 1e-5;
	int numIter = 0;
	int maxIter = 100;
	while (numIter < maxIter && error > minError) {
		double curError = 0.0;
		for (int i=0;i<peaks->numPeaks;i++) {
			double v1 = peaks->peakOrder[i]->v;
			double ptotal = 0.0;
			for (int j=0;j<peaks->numPeaks;j++) {
				double v2 = peaks->peakOrder[j]->v;
				int distIndex = abs(i-j);
				//int distance = distIndex*freqRes;
				double freq = dist->freq[distIndex];
				double expected = freq*v1*v2/avg*adjTotals[i]*adjTotals[j];
				if (i==j) {
					expected = freq*v1*adjTotals[i];
				}
				ptotal += expected;
			}
			double diff = ptotal-avg;
			double change = 1-diff/avg/dampFactor;
			if (change < 0.9) change = 0.9;
			if (change > 1.1) change = 1.1;
			newAdjTotals[i] = change*adjTotals[i];
			curError += diff*diff;
		}
		curError = sqrt(curError);
		fprintf(stderr, "\terror=%lf\n", curError);
		double* tmp = adjTotals;
		adjTotals = newAdjTotals;
		newAdjTotals = tmp;
		error = curError;
		numIter++;
	}
	for (int i=0;i<peaks->numPeaks;i++) {
		double v=peaks->peakOrder[i]->v;
		double simpleNorm = avg/(v+1e-5);
		fprintf(stderr, "%lf\t%lf\t%lf\t%lf\n",v,avg/(v+0.0001),adjTotals[i],v*adjTotals[i]);
		
		//peaks->peakOrder[i]->v *= adjTotals[i]/simpleNorm;
		peaks->peakOrder[i]->v *= simpleNorm/adjTotals[i];
		//peaks->peakOrder[i]->v = avg;
	}
...
void TagLibrary::makeHiCBgModelSimple(HiCBgModel* model,HiCparams* params) {
	model->totalRegions = (int) (((double)gsizeEstimate)/((double)params->superRes));
	model->totalModelReads = totalTags;
	model->totalReads = totalTags;
}
...
void TagLibrary::makeHiCBgModel(HiCBgModel* model,PeakLibrary* &peaks1, PeakLibrary* peaks2, 
	...
	double Navg = 0.0;
	for (int i=0;i<peaks1->numPeaks;i++) {
		if (peaks1->peakOrder[i]->v < model->minCoverage || peaks1->peakOrder[i]->v > model->maxCoverage) continue;
		cavg += peaks1->peakOrder[i]->v;
		Navg+=1.0;
	}
	if (peaks1 != peaks2) {
		for (int i=0;i<peaks2->numPeaks;i++) {
			if (peaks2->peakOrder[i]->v < model->minCoverage || peaks2->peakOrder[i]->v > model->maxCoverage) continue;
			cavg += peaks2->peakOrder[i]->v;
			Navg+=1.0;
		}
	}
	cavg /= Navg;
	model->adjAverage = cavg;
	fprintf(stderr, "\tAvg interactions per peak = %.1lf +/- %.1lf (%.1lf)\n", avg,std,cavg);
	...
	actionFlag = actionFlag | HIC_MASK_NORM_BALANCED_SEQDEPTH;
...
void TagLibrary::makeHiCHistogram(char* outputFile, PeakLibrary* peaks, int size, 
	...
	//fprintf(stderr, "\tHistogram resolution size: %d\n", resolution);
	...
	if (accuracy < 0.98) {
		...
		getPETagTotals(histPeaks,balanceEstFlag);
	...
	double Navg = 0.0;
	for (int i=0;i<histPeaks->numPeaks;i++) {
		if (histPeaks->peakOrder[i]->v < model->minCoverage || histPeaks->peakOrder[i]->v > model->maxCoverage) continue;
		cavg += histPeaks->peakOrder[i]->v;
		Navg += 1.0;
	}
	if (Navg > 0.0) cavg /= (Navg);
	model->adjAverage = cavg;
	fprintf(stderr, "\tAverage interaction count in regions = %.1lf +/- %.1lf (%.1lf without outliers)\n", avg,std,cavg);
	...
	if (model != NULL && !(actionFlag & HIC_MASK_LOGPVALUES) && actionFlag & HIC_MASK_NORM_DISTANCE) {
...
double** TagLibrary::makeHiCMatrix(PeakLibrary* peaks1, PeakLibrary* peaks2, 
	...
	double totalInteractions = totalTags;
	fprintf(stderr, "\tTotal Hi-C interactions: %ld\n", (long int)(totalInteractions/2.0));
	...
	if ((actionFlag & HIC_MASK_NORM_BALANCED_SEQDEPTH) && !(actionFlag & HIC_MASK_NORM_DISTANCE)) {
		fprintf(stderr, "\tSetting estimated matrix balancing flag to 1\n");
		balanceEstFlag=1;
		int mask = HIC_MASK_NORM_BALANCED_SEQDEPTH;
	...
	if (actionFlag & HIC_MASK_NORM_SEQDEPTH) {
		...
		//if ((actionFlag & HIC_MASK_NORM_BALANCED_SEQDEPTH) && model != NULL) {
		//	accuracy = peaks1->adjustPETagTotalsWithModel(model,1);
		...
		double Navg = 0.0;
		for (int i=0;i<numPeaks1;i++) {
			if (peakArray1[i]->v < model->minCoverage || peakArray1[i]->v > model->maxCoverage) continue;
			cavg += peakArray1[i]->v;
			Navg += 1.0;
		}
		localNumPeaks2 = numPeaks2;
		if (relativeFlag) localNumPeaks2 = numPeaks1;
		for (int i=0;i<localNumPeaks2;i++) {
			if (peakArray2[i]->v < model->minCoverage || peakArray2[i]->v > model->maxCoverage) continue;
			cavg += peakArray2[i]->v;
			Navg += 1.0;
		}
		if (Navg > 0.0) cavg /= (Navg);
...
void ChrTags::loadSequence(char* genomeDir) {
	...
	while (fgets(buffer,BUFFER,fp)!=NULL) {
		...
		if (goodSeq) {
			found=1;
			if (((unsigned int)(lineLen+chrLen+2)) >= likelyMaximum) break;
	...
	//fprintf(stderr, "buffer=%s\ncurName=%s\nchrLen=%d\nBUFFER=%d\nlikelySize=%ld\n\n", buffer, curName, chrLen,BUFFER,likelyMaximum);
	delete []buffer;
	delete []curName;
	if (found==0) {
		delete []seq;
...
void ChrTags::randomizeReadLevels() {
	loadTags();
	
	srand(time(NULL));

	Tag* newtags = new Tag[totalPositions];
	for (int i=0;i<totalPositions;i++) {
		newtags[i].copy(&(tags[i]));
		newtags[i].p = rand() % INT_MAX;
	}
	qsort(newtags,totalPositions,sizeof(Tag),&cmpTags);
	for (int i=0;i<totalPositions;i++) {
		newtags[i].p = tags[i].p;
		newtags[i].d = tags[i].d;
	}
	delete []tags;
	tags = newtags;
}
...
void ChrTags::findmCPeaks(PeakLibrary* peaks, int peakSize, char strand,int mCflag,
							double mCthresh,int minNumC, ChrTags* input,int randFlag) {
	loadTags();
	if (randFlag) randomizeReadLevels();
	if (input != NULL) {
		input->loadTags();
		if (randFlag) input->randomizeReadLevels();
	}
	...
	double* curValues = new double[maxValues];
	double* curInputValues = new double[maxValues];
	...
	if (mCflag == PEAKS_FIND_METHYLC || input != NULL) mCthresh *= -1;
	//fprintf(stderr, "mCthrsh= %lf\n", mCthresh);

	for (int i=0;i<numMaxTestPeaks;i++) {
		PeakmC* tp = new PeakmC();
		int start = i*(int)(((double)peakSize)/((double)resFactor));
		//fprintf(stderr, "%s\t%d\t%d\t%d\t%d\n",chr,start,i,peakSize,resFactor);
		...
		if (input != NULL) {
			for (;indexInput<input->totalPositions;indexInput++) {
				if (input->tags[indexInput].p < start) {
				} else {
					break;
				}
			}
		}
		int curIndexInput = indexInput;
		...
		for (int j=index;j<totalPositions;j++) {
			...
			if (tags[j].p >= end) break;


			// if input exists, check if this position is in input too.
			if (input != NULL) {
				int good = 0;
				for (int k=curIndexInput;k<input->totalPositions;k++) {
					if (input->tags[k].p < tags[j].p) {
						if (k==curIndexInput) curIndexInput++;
						continue;
					} else if (tags[j].p < input->tags[k].p) {
						break;
					} else {
						if (tags[j].d == input->tags[k].d) {
							good=1;
							curIndexInput = k;
							break;
						}
			...
			tp->numC +=1;
			tp->mCavg += tags[j].v;
			double score = tags[j].v;
			if (mCflag == PEAKS_FIND_METHYLC) score *= -1;

			curValues[tp->numC-1] = tags[j].v;

			if (input != NULL) {
				tp->mCavgInput += input->tags[curIndexInput].v;
				score = tags[j].v-input->tags[curIndexInput].v;
				if (mCflag == PEAKS_FIND_METHYLC) score *= -1;
				curInputValues[tp->numC-1] = input->tags[curIndexInput].v;
			}
		...
		//if (eIndex != -1 && tp->numC >= minC) {
			for (int j=0;j<tp->numC;j++) {
				tp->mCstd += (tp->mCavg-curValues[j])*(tp->mCavg-curValues[j]);
				tp->mCstd += (tp->mCavg-curValues[j])*(tp->mCavg-curValues[j]);
			}
			...
			if (input != NULL) {
				for (int j=0;j<tp->numC;j++) {
					tp->mCstdInput += (tp->mCavgInput-curInputValues[j])*(tp->mCavgInput-curInputValues[j]);
					tp->mCstdInput += (tp->mCavgInput-curInputValues[j])*(tp->mCavgInput-curInputValues[j]);
				}
				tp->mCstdInput /= (double)tp->numC;
				tp->mCstdInput = sqrt(tp->mCstdInput);
			}

			//set score (lower is better)
			if (mCflag == PEAKS_FIND_UNMETHYLC) {
				tp->score = tp->mCavg;
				if (input != NULL) {
					tp->score = tp->mCavg - tp->mCavgInput;
				}
			} else if (mCflag == PEAKS_FIND_METHYLC) {
				tp->score = -1*tp->mCavg;
				if (input != NULL) {
					tp->score = tp->mCavgInput - tp->mCavg;
				}
			}
	...
	double mCmedian = 0;
	if (numTestPeaks > 0) {
		mCmedian = sortPeaks[midPoint]->score;
	}
	...
	int glastEnd = -1;
	int lastN = 0;
	double lastmCscore = 0;
	double lastmCavg = 0;
	double lastmCavgInput = 0;
	int chrNumPeaks = 0;
	int badPeak = 1;
	for (int i=0;i<numTestPeaks;i++) {
		
		if (testPeaks[i]->score < mCthresh) {
		...
		//if (testPeaks[i]->mCavg+testPeaks[i]->mCstd < mCmedian-avgStd) {
			if (lastN > 0) {
				if (lastEnd+1>testPeaks[i]->start || badPeak == 0) {
					...
					lastN++;
					lastmCscore += testPeaks[i]->score;
					lastmCavg += testPeaks[i]->mCavg;
					lastmCavgInput += testPeaks[i]->mCavgInput;
				} else {
					...
					double reportStat = lastmCscore/lastN;
					if (mCflag == PEAKS_FIND_METHYLC) reportStat *= -1;
					peaks->addPeak(NULL,chr,glastStart,glastEnd,(glastStart+glastEnd)/2,STRAND_POSITIVE,reportStat,lastN,NULL,0,0);
					chrNumPeaks++;
...
void PeakmC::calcAvg() {
	...
	score = mCavg-mCavgInput;
}
PeakmC::~PeakmC() {
}

int cmpPeakmC(const void* a, const void* b) {
	double mCa = (*(PeakmC**)a)->score;
	double mCb = (*(PeakmC**)b)->score;
	if (mCa < mCb) return -1;
	if (mCa > mCb) return 1;
	return 0;
}
...
Tag* ChrTags::getRatioTags(int &ratioPositions, Tag* coverageTags, int coveragePositions, Tag* inputTags, 
				int inputPositions, double pseudoCounts,int subtractFlag) {
...
void ChrTags::printBedGraph(FILE* fp,double normFactor,char strand,int resolution,int negFlag,
				...
				int subtractFlag, double inputNormFactor, int inputFragLength, double normLength) {
	...
	if (input != NULL) {
		...
		Tag* ratioTags = getRatioTags(ratioPositions,coverageTags, coveragePositions, coverageTagsInput, 
								coveragePositionsInput,pseudoCounts,subtractFlag);		
/*
for (int i =0;i<coveragePositions;i++) {
...
void ChrTags::normalizeTagCountsLength(double** normLenTable) {

	loadTags();

	totalTags = 0.0;
	for (int i=0;i<totalPositions;i++) {
		double adjRatio = normLenTable[tags[i].len][2];
		tags[i].v *= adjRatio;
		totalTags += tags[i].v;
	}
...
void ChrTags::filterReadsWithOligo(char* genomeDirectory, char* filterOligo, 
															int filterOffset, int filterFlag) {
	loadTags();
	loadSequence(genomeDirectory);
	if (seq == NULL) {
	...
	int olen = strlen(filterOligo);
	char* fragSeq = new char[olen*2];
	int oligoStart = filterOffset;
	int oligoEnd = filterOffset+olen-1;
	...
	for (int i=0;i<totalPositions;i++) {
		...
		getSequence(fragSeq,start,end,tags[i].d);
//fprintf(stderr, "%s %d %d %s %d %d %d %d\n", chr,tags[i].p,tags[i].d,fragSeq, start,end,oligoStart, oligoEnd);
		if (strcmp(fragSeq, filterOligo)==0) {
			//match
			if (filterFlag == READ_FILTER_REMOVE) {
				tags[i].v = 0.0;
				tags[i].p = -1;
			}
		} else {
			// not a match
			if (filterFlag == READ_FILTER_KEEP) {
				tags[i].v = 0.0;
				tags[i].p = -1;
			}
		}
...
void ChrTags::removeNegPositions() {
	double newTotal = 0.0;
	int newTotalPositions = 0;
	Tag* newtags = new Tag[totalPositions];
	for (int i=0;i<totalPositions;i++) {
		if (tags[i].p < 0) continue;
		newtags[newTotalPositions++].copy(&(tags[i]));
		newTotal += tags[i].v;
	}
	delete []tags;
	tags = newtags;
...
void ChrTags::makeHiCMatrix(PeakLibrary* peaks1, int &peakIndex1, PeakLibrary* peaks2,
	...
	int relativeFlag = params->relativeFlag;
	double normTotal = params->totalNormReads;
	double gsize = params->gsize;
	...
	//----- Check on the chromosome read coverage
	double chr1CoverageRatio = 1.0;
	double chr2CoverageRatio = 1.0;
	if (actionFlag & HIC_MASK_NORM_SEQDEPTH) {
		double genomeWideAvg = model->totalReads / ((double)model->totalRegions);
		//double genomeWideAvg = model->adjAverage;
		double avg = 0.0;
		double n = 0.0;
		for (int i=peakIndex1;i<lastIndex1;i++) {
			if (peakArray1[i]->v < model->minCoverage || peakArray1[i]->v > model->maxCoverage) continue;
			avg += peakArray1[i]->v;
			n += 1.0;
		}
		if (n > 0.0) avg /= n;
		//fprintf(stderr, "\t%lf\t%lf\n", avg, genomeWideAvg);
		chr1CoverageRatio = avg/genomeWideAvg;
		fprintf(stderr, "\t\t%s (avg coverage: %.1lf, coverage ratio=%.3lf)\n", chr,avg,chr1CoverageRatio);
		//fprintf(stderr, "\t\t(%.1lf total reads, %d total regions)\n",model->totalReads,model->totalRegions);
	} else {
		fprintf(stderr, "\t\t%s\n", chr);
	}
	loadTags();
	int finalPosition = 0;
	if (petags != NULL && totalPositions > 0) finalPosition = petags[totalPositions-1].p1;
	...
	if (actionFlag & HIC_MASK_CREATEMODEL) {
		...
		chrModel = (HiCBgModelChr*)model->chrs->search(chr);
		if (chrModel == NULL && (actionFlag & (HIC_MASK_NORM_DISTANCE | HIC_MASK_NORM_BALANCED_SEQDEPTH))) {
	...
	HiCBgModelChr* ifc = new HiCBgModelChr(resolution);
	char* lastChr2 = NULL;
	...
	for (int i=0;i<curNumPeaks1;i++) {
		...
		if (gil != NULL) {
			...
			if (actionFlag & HIC_MASK_INTERACTION4CBEDFILE && gil != NULL && gil->bedFile != NULL
				...
				for (int k=0;k<count;k++) {
					//double v = curPETags[k]->v;
					//fprintf(stderr, "%lf\t%d\t%d\n", v,params->logFlag, actionFlag & HIC_MASK_NORM_SEQDEPTH);
					//if (params->logFlag==0 && actionFlag & HIC_MASK_NORM_SEQDEPTH) {
					//	curPETags[k]->v = pow(2,v);
					//}
		...
		for (int j=0;j<numPeaks2;j++) {		
			...
			//----- Check on the other chromosome read coverage
			if (actionFlag & HIC_MASK_NORM_SEQDEPTH && lastChr2 != NULL && strcmp(lastChr2, chr2)!=0) {

				chr2CoverageRatio = 1.0;
				double genomeWideAvg = model->totalReads / ((double)model->totalRegions);
			//double genomeWideAvg = model->adjAverage;
				double avg = 0.0;
				double n = 0.0;

				for (int k=j;k<numPeaks2;k++) {
					if (strcmp(peakArray2[k]->chr,chr2)!=0) break;
					if (peakArray2[k]->v < model->minCoverage || peakArray2[k]->v > model->maxCoverage) continue;
					avg += peakArray2[k]->v;
					n += 1.0;
				}
				if (n > 0.0) avg /= n;
				//fprintf(stderr, "\t%lf\t%lf\n", avg, genomeWideAvg);
				chr2CoverageRatio = avg/genomeWideAvg;
				//fprintf(stderr, "\t\t%s (avg coverage: %.1lf, coverage ratio=%.3lf)\n", chr,avg,chr2CoverageRatio);
				//fprintf(stderr, "\t\t(%.1lf total reads, %d total regions)\n",model->totalReads,model->totalRegions);
			}
			lastChr2 = chr2;
			...
			if (normTotal > 0.0) {
				p2Stats[j] *= (normTotal/totalInteractions);
			}
			...
			if (actionFlag & HIC_MASK_NORM_SEQDEPTH) {
				...
				double zscore = 0.0;
				double p1total = peak1total;
				double p2total = peak2total;
				double simpleExpect = (p1total) * (p2total) / totalInteractions;
				if (model!=NULL) {
					if (p1total < model->minExpectCoverage) p1total = model->minExpectCoverage;
					if (p2total < model->minExpectCoverage) p2total = model->minExpectCoverage;
				}
				if (actionFlag & HIC_MASK_NORM_DISTANCE) {
					// need to adjust values so that the distance normalization is consistent with the
					// total reads per location
					expect = (p1total*peakArray1[pIndex]->focusRatio) * (p2total*peakArray2[j]->focusRatio);
				...
				} else if (actionFlag & HIC_MASK_NORM_BALANCED_SEQDEPTH) {
					expect = (p1total*peakArray1[pIndex]->focusRatio/model->totalModelReads) 
													* (p2total*peakArray2[j]->focusRatio/model->totalModelReads);
					expect *= model->totalReads;
					esize = ((double)model->totalRegions);
				} else {
					expect = (p1total) * (p2total);
					expect /= totalInteractions;
					esize = gsize/((double)resolution);
				}
				...
				if (!(actionFlag & HIC_MASK_CREATEMODEL)) {
					if (actionFlag & HIC_MASK_RAWANDEXPECTED) {
						...
						if (actionFlag & HIC_MASK_NORM_DISTANCE) {
							...
							p2Stats[j] = ((HIC_RATIO_PSEUDO_COUNT+interactions)
												/(HIC_RATIO_PSEUDO_COUNT+expect));
						} else if (actionFlag & HIC_MASK_NORM_SEQDEPTH) {
							//p2Stats[j] = interactions/expect;
							double expectedReadsPerPosition = totalInteractions / (esize*esize);
							//if chr has less coverage, don't 'inflate' the read counts i.e. chrX/chrY

							if (normTotal > 0.0) {
								expectedReadsPerPosition = normTotal / (esize*esize);
							}
							if (chr1CoverageRatio*chr2CoverageRatio < 0.75) {
								expectedReadsPerPosition *= chr1CoverageRatio*chr2CoverageRatio;
							}
							if (expect < expectedReadsPerPosition/100.0) {
								expect = expectedReadsPerPosition/100.0;
							}
							double factor = expect/expectedReadsPerPosition;
							if (model != NULL && factor < model->minExpectFraction) {
								//factor = model->minExpectFraction;
							}
							p2Stats[j] = interactions/factor;
							//p2Stats[j] *= expectedReadsPerPosition;
...
//fprintf(stderr, "%s\t%d\t%s\t%d\t%lf\n", chr, start1,  chr2, curStart, p2Stats[j]);
		...
		//
		//using p2Stats for read counts.
		if (model != NULL && ifc != NULL) {
			...
			if (rtotal > 1e-10) {
				for (int j=0;j<numPeaks2;j++) {		
					...
					char* chr2 = peakArray2[j]->chr;
					int curStart = peakArray2[j]->start;
					double vv = p2Stats[j]/rtotal;
					if (strcmp(chr2,chr1)==0) {
						double idist = fabs((double)(curStart-start1));
						int intIndex = (int) floor(idist/(double)resolution+0.5);
						ifc->expected[intIndex] += vv;
						ifc->expectedN[intIndex] += 1.0;
						//model->currentRegions->expected[intIndex] += vv;
						//model->currentRegions->expectedN[intIndex] += 1.0;
					} else {
						ifc->interChr += vv;
						ifc->interChrN += 1.0;
						//model->currentRegions->interChr += vv;
						//model->currentRegions->interChrN += 1.0;
						interTotal += vv;
					}
				}
				//fprintf(stderr, "  - %s:%d", chr1,start1);
				//fprintf(stderr, "    interTotal = %lf\n", interTotal);
				ifc->totalRegions++;
				//model->currentRegions->totalRegions++;
			}
		}
		//	fprintf(stderr, "asdfasdfdf\n");
		if (1 && gil != NULL && actionFlag & HIC_MASK_INTERACTIONSTATSFILES && i!=curNumPeaks1-1) {
						
			int dlrDistance = gil->dlrDistance;
			double totalInterChrInteractions = 0.0;
			double totalAllInteractions = 0.0;
			double totalLocalInteractions = 0.0;
			double totalCohesinInteractions = 0.0;
			double totalTranscriptionInteractions = 0.0;
			...
			for (int j=0;j<numPeaks2;j++) {		
				...
				if (strcmp(chr2,chr1)==0) {
					double idist = fabs((double)(curStart-start1));
					if (idist < HIC_DLR_DEFAULT_MINDISTANCE) {
						totalLocalInteractions += vv;
					} else if (idist < dlrDistance) {
						totalCohesinInteractions += vv;
					} else {
						totalTranscriptionInteractions += vv;
					}	
				} else {
					totalInterChrInteractions += vv;
				}
				totalAllInteractions += vv;
			}
			...
			double r = 0.0;
			//double r2 = 0.0;
			double cohesinRatio = log((totalTranscriptionInteractions+1.0)/(totalCohesinInteractions+1.0))/log(2.0);
			...
			//
			int midpoint = (start1+end1)/2;
			int upres = resolution/2;
			int downres = resolution/2;
			if (pIndex-1 >= 0 && strcmp(peakArray1[pIndex]->chr,peakArray1[pIndex-1]->chr)==0) {
				int ss = (start1-peakArray1[pIndex-1]->start)/2;
				if (ss > 0 && ss < upres) upres = ss;
			}
			if (pIndex+1 < numPeaks1 && strcmp(peakArray1[pIndex]->chr,peakArray1[pIndex+1]->chr)==0) {
				int ss = (peakArray1[pIndex+1]->start-start1)/2;
				if (ss > 0 && ss < downres) downres = ss;
				//fprintf(stderr, "Here? %d %d\n", ss, downres);
			}
			//fprintf(stderr, "upres=%d dnres=%d\n", upres, downres);
			int s = midpoint-upres;
			int e = midpoint+downres;

			if (s < 0) s = 0;
			if (e > finalPosition) e = finalPosition;
			pthread_mutex_lock(&(gil->mutex));
			fprintf(gil->bedFileInterFrac,"%s\t%d\t%d\t%lf\n",chr1,s,e,r);
			fprintf(gil->bedFileLocalFrac,"%s\t%d\t%d\t%lf\n",chr1,s,e,cohesinRatio);
			fprintf(gil->bedFileCoverage,"%s\t%d\t%d\t%lf\n",chr1,s,e,totalAllInteractions);
			pthread_mutex_unlock(&(gil->mutex));
			//continue;
		}
...
//							fprintf(stderr, "std %lf\t%lf\n",normV,1.0*w);
	...
	if (model != NULL && gil != NULL && ifc != NULL) {
		pthread_mutex_lock(&(gil->mutex));
		
		for (int i=0;i<ifc->maxIndex;i++) {
			model->currentRegions->expected[i] += ifc->expected[i];
			model->currentRegions->expectedN[i] += ifc->expectedN[i];
		}
		model->currentRegions->interChr += ifc->interChr;
		model->currentRegions->interChrN += ifc->interChrN;
		model->currentRegions->totalRegions += ifc->totalRegions;
		pthread_mutex_unlock(&(gil->mutex));
		delete ifc;
	}
...
void HiCBgModelChr::normalizeCurve() {
	...
	total+=interChr;
	if (1) {
		total = (double) totalRegions;
	}
	//fprintf(stderr, "total=%lf (maxIndex=%d, maxUsedIndex=%d, totalRegions=%d)\n", total,maxIndex,maxUsedIndex,totalRegions);
	if (total > 0.0) {
		interChr /= total;
...
void HiCBgModel::init() {
	...
	modelError=0.0;
	fullModelFlag = 0;
	minExpectCoverage = 0.0;
	minExpectFraction = 0.5;
	minExpectCoverage=0.0;
	numPeaks1=0;
	numPeaks2=0;
	expectMatrix=NULL;
...
void HiCBgModel::initializeCurrentRegions(int resolution) {
	if (currentRegions != NULL) {
		delete currentRegions;
	}
	currentRegions = new HiCBgModelChr(resolution);
	currentRegions->initialize();
}
...
void HiCBgModel::setCoverageLimits(double a, double s) {
	...
	if (stdFilter < 0 || stdCoverage < 0) {
		//fprintf(stderr, "MAX\n");
		maxCoverage = DBL_MAX;
	}
	double altMin = a*minFilter;
	minCoverage = altMin;
	if (minExpectFraction < 1e-6) minExpectFraction = 1e-6;
	minExpectCoverage = a*minExpectFraction;
...
void HiCBgModel::printCurrentRegions(FILE* fp) {
	if (currentRegions == NULL) {
		fprintf(fp, "Error- currentRegions doesn't exist! (null)\n");
		return;
	}
	fprintf(fp, "Distance\tInteraction Frequency (interChr=%le)\n", currentRegions->interChr);
	for (int i=0;i<currentRegions->maxUsedIndex;i++) {
		fprintf(fp, "%d\t%le\n",i*currentRegions->res,currentRegions->expected[i]);
	}
}
...
void HiCBgModel::normalize() {
	...
	for (int i=0;i<chrs->total;i++) {
		...
		interChrN += hcbm->interChrN;
		//fprintf(stderr, "Normalizing: %s\t%lf\t%lf\n", keys[i],hcbm->interChr, hcbm->interChrN);
		hcbm->normalize();
		delete [](keys[i]);
	}
	delete []keys;
...
void GenomeInteraction::print2dBED(FILE* fp) {
	fprintf(fp,"%s\t%d\t%d\t%s\t%d\t%d",chr1,start1,end1,chr2,start2,end2);
	fprintf(fp, "\t0,0,250");
...
GenomeInteractionLibrary::GenomeInteractionLibrary() {
	...
	dlrDistance = HIC_DLR_DEFAULT_DISTANCE;
	threshold = 1e-5;
	zscoreThreshold = 1.0;
	minDist = 0;
	maxDist = -1;
...
int GenomeInteractionLibrary::read2dBED(char* file) {
	if (file == NULL) return -1;
	FILE* fp = fopen(file,"r");
	if (fp == NULL) {
		fprintf(stderr, "!!! Couldn't open 2D BED interaction file: %s !!!\n", file);
		return -1;
	}
	char* buf = new char[BUFFER];
	char** cols = new char*[BUFFER];
	...
	while (fgets(buf, BUFFER, fp) != NULL) {
		...
		if (numCols < 6) continue;
		if (buf[0] == '#') continue;
		if (checkInt(cols[1]) || checkInt(cols[2]) || checkInt(cols[4]) || checkInt(cols[5])) {
			continue;
		} else {
		}
		...
		char* chr1 = cols[0];
		int start1 = -1;
		sscanf(cols[1],"%d",&start1);
		int end1 = -1;
		sscanf(cols[2],"%d",&end1);
		char strand1 = STRAND_POSITIVE;
		float v1 = 0.0;
		int info = 0;	
		Peak* p1 = NULL;
		p1 = peakList->checkForPeak(name1, chr1,start1,end1,strand1,info);
		if (p1 == NULL) {
			p1 = peakList->addPeak(name1,chr1,start1,end1,(start1+end1)/2,strand1,v1,0.0,NULL,0,0);
		}
		resolutionEstimate += end1-start1;
		...
		char* chr2 = cols[3];
		int start2 = -1;
		sscanf(cols[4],"%d",&start2);
		int end2 = -1;
		sscanf(cols[5],"%d",&end2);
		char strand2 = STRAND_POSITIVE;
		float v2 = 0.0;

		Peak* p2 = NULL;
		p2 = peakList->checkForPeak(name2, chr2,start2,end2,strand2,info);
		if (p2 == NULL) {
			p2 = peakList->addPeak(name2,chr2,start2,end2,(start2+end2)/2,strand2,v2,0.0,NULL,0,0);
		}

		double intScore = 0.0;
		if (numCols > 7) sscanf(cols[7],"%lf",&intScore);
		double intExpect = 0.0;
		if (numCols > 8) sscanf(cols[8],"%lf",&intExpect);
		resolutionEstimate += end2-start2;
		resEstN ++;

		GenomeInteraction* gi = new GenomeInteraction(p1, p2);
		gi->interactions = intScore;
		gi->expected = intExpect;
		if (strcmp(chr1,chr2)!=0) {
			sprintf(buf,"int-%s:%d-%s:%d",chr1,start1,chr2,end2);
		} else if (start2 < start1) {
			sprintf(buf,"%s:%d-%d",chr1,start2,end1);
		} else {
			sprintf(buf,"%s:%d-%d",chr1,start1,end2);
		}
		gi->setName(buf);
		addInteraction(gi);
...
void GenomeInteractionLibrary::print2dBED(FILE* fp) {
	for (int i=0;i<numInteractions;i++) {
		interactions[i]->print2dBED(fp);
	}
}
